"""
Run the rebelity-govity analysis for cz-psp-data-2025-202x.

Steps:
  1. Ensure votes.csv and vote_events.json exist (downloads from B2 if needed)
  2. Run rebelity_govity.py (from legislature-data-analyses)
  3. Run output_flourish_table.py to produce the Flourish CSV

Outputs:
  analyses/rebelity-govity/outputs/rebelity_govity.json
  analyses/rebelity-govity/outputs/rebelity_govity_flourish_table.csv

Usage:
  python scripts/analyses/run_rebelity_govity.py \\
      --script /path/to/legislature-data-analyses/rebelity-govity/rebelity_govity.py \\
      --flourish-script /path/to/legislature-data-analyses/rebelity-govity/outputs/output_flourish_table.py \\
      [--use-current-members]

The --script and --flourish-script arguments are required because the
rebelity-govity analysis lives in a separate repository (legislature-data-analyses)
...
_DEFINITION  = _REPO_ROOT / "analyses" / "rebelity-govity" / "rebelity_govity_definition.json"
...
_OUTPUT_JSON = _REPO_ROOT / "analyses" / "rebelity-govity" / "outputs" / "rebelity_govity.json"
_OUTPUT_CSV  = _REPO_ROOT / "analyses" / "rebelity-govity" / "outputs" / "rebelity_govity_flourish_table.csv"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to rebelity_govity.py from the legislature-data-analyses repository.",
    )
    parser.add_argument(
        "--flourish-script", required=True, dest="flourish_script",
        help="Path to rebelity-govity/outputs/output_flourish_table.py from legislature-data-analyses.",
    )
