"""
Run the rebelity analysis for cz-psp-data-2025-202x.

Steps:
  1. Ensure votes.csv and vote_events.json exist (downloads from B2 if needed)
  2. Run rebelity.py (from legislature-data-analyses)
  3. Run output_flourish_table.py to produce the Flourish CSV

Outputs:
  analyses/rebelity/outputs/rebelity.json
  analyses/rebelity/outputs/rebelity_flourish_table.csv

Usage:
  python scripts/analyses/run_rebelity.py \\
      --script /path/to/legislature-data-analyses/rebelity/rebelity.py \\
      --flourish-script /path/to/legislature-data-analyses/rebelity/outputs/output_flourish_table.py

The --script and --flourish-script arguments are required because the
rebelity analysis lives in a separate repository (legislature-data-analyses)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to rebelity.py from the legislature-data-analyses repository.",
    )
    parser.add_argument(
        "--flourish-script", required=True, dest="flourish_script",
        help="Path to rebelity/outputs/output_flourish_table.py from legislature-data-analyses.",
    )
    ...
    _ensure_dt_schema_file(
        filename="rebelity-definition.dt.analyses.json",
        url="https://michalskop.github.io/legislature-data-standard/dt.analyses/rebelity-definition/latest/schemas/rebelity-definition.dt.analyses.json",
        schema_dir="/tmp/legislature-data-standard/dist/dt.analyses/rebelity-definition/latest/schemas",
    )
