...
SCHEMA_URL = "https://michalskop.github.io/legislature-data-standard/dt/0.1.0/schemas/motions.dt.json"
...
def validate_motions(json_path: Path, schema_url: str = SCHEMA_URL) -> None:
    schema = _load_schema(schema_url)
    item = schema.get("definitions", {}).get("DtMotions", {}).get("items", {})
...
def main() -> None:
    ...
    parser.add_argument("--json", default="work/standard/motions_sample.json")
    parser.add_argument("--schema-url", default=SCHEMA_URL)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    validate_motions(Path(args.json), args.schema_url)
